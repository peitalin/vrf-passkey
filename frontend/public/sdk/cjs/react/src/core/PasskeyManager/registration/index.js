...
var createAccountTestnetFaucet = require('./createAccountTestnetFaucet.js');
 ...
 */
async function generateBootstrapVrfChallenge(webAuthnManager, nearAccountId, blockHeight, blockHashBytes) {
    console.log('Generating VRF keypair for registration');
    // Generate VRF keypair and persist in worker memory
    const vrfResult = await webAuthnManager.generateVrfKeypair(true, // saveInMemory: true - this VRF keypair is persisted in worker memory until PRF encryption
    {
 ...
 */
async function registerPasskey(context, nearAccountId, options) {
    ...
    try {
        ...
        return await handleRegistration(context, nearAccountId, onEvent, onError, hooks);
    }
    catch (err) {
...
const validateRegistrationInputs = (nearAccountId, onEvent, onError, hooks) => {
    onEvent?.({
        step: 1,
        phase: 'webauthn-verification',
        status: 'progress',
    ...
    const validation$1 = validation.validateNearAccountId(nearAccountId);
    if (!validation$1.valid) {
        const error = new Error(`Invalid NEAR account ID: ${validation$1.error}`);
        onError?.(error);
        throw error;
    }
 ...
 */
async function handleRegistration(context, nearAccountId, onEvent, onError, hooks) {
    ...
    try {
        ...
        const vrfChallenge = await generateBootstrapVrfChallenge(webAuthnManager, nearAccountId, blockHeight, blockHashBytes);
        ...
        const { credential } = await webAuthnManager.touchIdPrompt.generateRegistrationCredentialsAndPrf({
            nearAccountId,
        ...
        const accountCreationResult = await createAccountTestnetFaucet.createAccountTestnetFaucet(nearAccountId, keyGenResult.publicKey, onEvent);
        ...
        let contractTransactionId = null;
        ...
        try {
            await webAuthnManager.atomicOperation(async (db) => {
                ...
                const credentialId = encoders.bufferEncode(credential.rawId);
                const response = credential.response;
                await webAuthnManager.storeAuthenticator({
                    nearAccountId,
                    credentialID: credentialId,
                    credentialPublicKey: await webAuthnManager.extractCosePublicKey(encoders.bufferEncode(response.attestationObject)),
 ...
 */
async function waitForAccessKey(nearRpcProvider, nearAccountId, nearPublicKey, maxRetries = 10, delayMs = 1000) {
    console.log(`Waiting for access key to be available for ${nearAccountId}...`);
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const accessKeyInfo = await nearRpcProvider.viewAccessKey(nearAccountId, nearPublicKey);
