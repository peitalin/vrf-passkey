...
class PasskeyClientDBManager {
    ...
    async getDB() {
        ...
        this.db = await idb.openDB(this.config.dbName, this.config.dbVersion, {
    ...
    validateNearAccountId(nearAccountId) {
        return validation.validateNearAccountId(nearAccountId);
    }
    /**
    ...
    extractUsername(nearAccountId) {
        ...
        if (!validation$1.valid) {
            throw new Error(`Invalid NEAR account ID: ${validation$1.error}`);
        }
...
exports.PasskeyClientDBManager = PasskeyClientDBManager;
