...
async function validateActionInputs(context, nearAccountId, actionArgs, options) {
    ...
    try {
        ...
        if (actionArgs.type === actions.ActionType.FunctionCall && (!actionArgs.methodName || actionArgs.args === undefined)) {
            throw new Error('Missing required parameters for function call: methodName or args');
        }
        if (actionArgs.type === actions.ActionType.Transfer && !actionArgs.amount) {
...
async function verifyVrfAuthAndSignTransaction(context, nearAccountId, transactionContext, actionArgs, options) {
    ...
    try {
        ...
        let signingResult;
        if (actionArgs.type === actions.ActionType.Transfer) {
        ...
        else if (actionArgs.type === actions.ActionType.FunctionCall) {
...
async function broadcastTransaction(signingResult, actionArgs, options) {
    ...
    try {
        ...
        const rpcResponse = await fetch(config.RPC_NODE_URL, {
            method: 'POST',
            ...
            body: JSON.stringify({
                ...
                params: {
                    ...
                    wait_until: rpc.DEFAULT_WAIT_STATUS.executeAction
                }
            })
        });
