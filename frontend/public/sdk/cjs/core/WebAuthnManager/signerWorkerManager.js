...
var signerWorker = require('../types/signer-worker.js');
var actions = require('../types/actions.js');
...
class SignerWorkerManager {
    ...
    async executeWorkerOperation({ message, onEvent, timeoutMs = 30000 // 30s
        ...
        return new Promise((resolve, reject) => {
            ...
            worker.onmessage = (event) => {
                ...
                if (response.type === signerWorker.WorkerResponseType.VERIFICATION_PROGRESS ||
                    response.type === signerWorker.WorkerResponseType.SIGNING_PROGRESS ||
                    response.type === signerWorker.WorkerResponseType.REGISTRATION_PROGRESS) {
                    const payload = response.payload;
                    onEvent?.(payload);
                    return; // Continue listening for more messages
                }
                // Handle completion messages
                if (response.type === signerWorker.WorkerResponseType.VERIFICATION_COMPLETE) {
                ...
                if (response.type === signerWorker.WorkerResponseType.SIGNING_COMPLETE ||
                    response.type === signerWorker.WorkerResponseType.REGISTRATION_COMPLETE) {
                    clearTimeout(timeoutId);
                    worker.terminate();
                ...
                // Handle errors
                if (response.type === signerWorker.WorkerResponseType.ERROR) {
                    clearTimeout(timeoutId);
                    worker.terminate();
    ...
    async deriveNearKeypairAndEncrypt(credential, nearAccountId) {
        ...
        try {
            ...
            const response = await this.executeWorkerOperation({
                message: {
                    type: signerWorker.WorkerRequestType.DERIVE_NEAR_KEYPAIR_AND_ENCRYPT,
                    payload: {
                        prfOutput: encoders.bufferEncode(prfOutput),
                        nearAccountId: nearAccountId,
                        attestationObjectBase64url: encoders.bufferEncode(attestationObject.attestationObject)
                    }
                }
            });
            if (signerWorker.isEncryptionSuccess(response)) {
    ...
    async decryptPrivateKeyWithPrf(touchIdPrompt, nearAccountId, authenticators) {
        try {
            ...
            const response = await this.executeWorkerOperation({
                message: {
                    type: signerWorker.WorkerRequestType.DECRYPT_PRIVATE_KEY_WITH_PRF,
                    payload: {
                        nearAccountId: nearAccountId,
                        prfOutput: encoders.bufferEncode(prfOutput)
                    }
            ...
            if (signerWorker.isDecryptionSuccess(response)) {
    ...
    async extractCosePublicKey(attestationObjectBase64url) {
        ...
        const response = await this.executeWorkerOperation({
            message: {
                type: signerWorker.WorkerRequestType.EXTRACT_COSE_PUBLIC_KEY,
                payload: {
                    attestationObjectBase64url
                }
            }
        });
        if (signerWorker.isCoseKeySuccess(response)) {
    ...
    async validateCoseKey(coseKeyBytes) {
        ...
        const response = await this.executeWorkerOperation({
            message: {
                type: signerWorker.WorkerRequestType.VALIDATE_COSE_KEY,
                payload: {
                    coseKeyBytes: Array.from(coseKeyBytes)
                }
            }
        });
        if (signerWorker.isCoseValidationSuccess(response)) {
    ...
    async checkCanRegisterUser({ vrfChallenge, webauthnCredential, contractId, onEvent, }) {
        try {
            ...
            const response = await this.executeWorkerOperation({
                message: {
                    type: signerWorker.WorkerRequestType.CHECK_CAN_REGISTER_USER,
                    payload: {
                        vrfChallenge,
                        webauthnCredential: signerWorker.serializeRegistrationCredentialAndCreatePRF(webauthnCredential),
                        contractId,
                        nearRpcUrl: config.RPC_NODE_URL
                    }
                },
                onEvent,
                timeoutMs: 60000 // Longer timeout for contract verification
            });
            if (signerWorker.isRegistrationSuccess(response)) {
    ...
    async signVerifyAndRegisterUser({ vrfChallenge, webauthnCredential, contractId, signerAccountId, nearAccountId, publicKeyStr, nearRpcProvider, onEvent, }) {
        try {
            ...
            const response = await this.executeWorkerOperation({
                message: {
                    type: signerWorker.WorkerRequestType.SIGN_VERIFY_AND_REGISTER_USER,
                    payload: {
                        vrfChallenge,
                        webauthnCredential: signerWorker.serializeRegistrationCredentialAndCreatePRF(webauthnCredential),
                        contractId,
    ...
    async signTransactionWithActions(payload, onEvent) {
        try {
            ...
            payload.actions.forEach((action, index) => {
                try {
                    signerWorker.validateActionParams(action);
                }
                catch (error) {
            ...
            const response = await this.executeWorkerOperation({
                message: {
                    type: signerWorker.WorkerRequestType.SIGN_TRANSACTION_WITH_ACTIONS,
                    payload: {
                        ...
                        webauthnCredential: signerWorker.serializeCredentialAndCreatePRF(payload.webauthnCredential),
                        nearRpcUrl: config.RPC_NODE_URL
                    }
                },
            ...
            if (response.type === signerWorker.WorkerResponseType.SIGNING_COMPLETE && response.payload.success) {
    ...
    async signTransferTransaction(payload, onEvent) {
        try {
            ...
            signerWorker.validateActionParams(transferAction);
            const response = await this.executeWorkerOperation({
                message: {
                    type: signerWorker.WorkerRequestType.SIGN_TRANSFER_TRANSACTION,
                    payload: {
                        nearAccountId: payload.nearAccountId,
                        receiverId: payload.receiverId,
