import type { AccountServiceConfig } from './types';

export function validateConfigs(config: AccountServiceConfig): void {
  ...
  for (const key of requiredEnvVars) {
    if (!config[key as keyof AccountServiceConfig]) {
