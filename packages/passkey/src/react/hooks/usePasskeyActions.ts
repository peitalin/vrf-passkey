...
export const usePasskeyActions = (
  ...
  const executeDirectActionViaWorker = useCallback(async (
    ...
    try {
      ...
      if (!serializableActionForContract.receiver_id || !serializableActionForContract.method_name || !serializableActionForContract.args) {
        throw new Error('Missing required action parameters: receiver_id, method_name, or args');
      }
